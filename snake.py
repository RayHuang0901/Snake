...
def food(canvas, snake_list) :
    #隨機生成的位置(x1, y1)
    global Column, Row, Food_coord,Rock_coord,maxFood
    global game_map
    if len(Food_coord)==maxFood :
        return
    for i in range(len(Food_coord),maxFood):
        while(True):
...
def start(score=0):
    global Direction,Food_coord,score_label,maxFood,over_label,tempLong
...
def rule():
    ...
    label_12 = tk.Label(rulepage ,text = "尖頭:可以破壞石頭")
    label_13 = tk.Label(rulepage ,text = "大豐收:產生目前生成量的兩倍果實")
    label_14 = tk.Label(rulepage ,text = "幸運:期間內只發動正面效果")
    
    label_15 = tk.Label(rulepage ,text = "負面暫時效果:")
    label_16 = tk.Label(rulepage ,text = "爆發成長:將長度變成兩倍")
    label_17 = tk.Label(rulepage ,text = "加速:速度加快三分之一")
    label_18 = tk.Label(rulepage ,text = "方向喪失:控制鍵上下顛倒，左右相反")
    label_19 = tk.Label(rulepage ,text = "大歉收:目前生成量減半")
